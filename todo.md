...
- [ ] `[span-api]` Give the Isar span parsing a supported import surface
      ...
      cartouches and non-Isar regions, and it is reachable today only by
      running the CLI and parsing the report back — so a consumer that wants
      *spans* rather than a *report* either imports a private name or
      reimplements it worse.  Both are happening in the upstream
      formalization project: one script imports `_parse_one`, another
      hand-rolls a 20-keyword `DECL_RE` anchored at `^` (so nothing inside a
      `context` or `locale` is visible), a `\<close>\s*$` cartouche end that
      is wrong for a nested cartouche, and no comment awareness — a strictly
      worse parser standing next to a better one, deleting lines.
      The precedent is settled and points one way: `isabelle-layout` exists
      because consumers needed the ROOT / session / header parsing, and the
      answer was neither "reimplement" nor "import a private name".

      **The ask is bigger than the need, and that is the useful finding.**
      The issue proposes re-exporting ten line-scanners plus making
      `_attach_preambles` and `_proof_extent` public.  But their RESULTS are
      already fields on `Entry`, which `_parse_one` populates — measured on a
      fixture with a `text` preamble, a structured proof and a `context`
      block:

          documented   thy_line=8 decl_end=10 proof_line=11 thy_end=14
                       body_end=14 preamble=(5, 6)

      `preamble=(5, 6)` is exactly what `delete-lemmas.py` gets wrong, and
      `proof_line`/`body_end_line` are the proof extent that "is not derivable
      from the entry span alone".  Both are already there.  So the minimal
      honest surface is roughly THREE names, not twelve:

          parse_theory  (today `_parse_one`)   Entry   TheorySection

      and `_parse_one(thy, path, lines=None)` already accepts pre-read source,
      so a consumer can parse a buffer without a file — the `-` stdin path
      proves it.  Export the dish, not the ingredients: the twelve
      public-by-naming scanners in `parsing` are line-oriented helpers whose
      composition is the thing worth depending on, and pinning them
      individually would freeze the internals this tool changes most often.

      Sharper than the issue puts it: the current import is
      `from isabelle_query.cli import _parse_one`, and `cli` only has that
      name as a **re-export** (`cli.py:96`, with a comment saying it is kept
      so the tests can reach it).  The consumer is depending on a private name
      re-exported for the test suite's convenience — two hops, neither
      load-bearing for it.

      Open decisions, in the order they need answering:
        - **Shape.**  `__all__` in `__init__.py`, an `isabelle_query.api`
          module, or a fourth package.  Lean to the smallest that names a
          stable entry point — carving out layout was the breaking half of
          0.7.0, and a package split is only worth repeating for a consumer
          that needs the piece standalone, which nothing does yet.
        - **What stability it carries**, said in one sentence, under a 0.x
          policy where breaking changes take the minor slot.  Exporting a
          name is the obligation; the README paragraph is only the notice.
        - **Pin it with a test**, the reciprocal of `test_layout_surface.py`:
          that file pins the ten layout names we reach for and fails on any
          private import anywhere in the repo.  The same shape pointed the
          other way makes removing an exported name a deliberate act rather
          than a silent break.
        - **Cost to note:** `__init__.py` currently imports nothing, so
          `import isabelle_query` is free.  Re-exporting from `parsing` makes
          it pull the parser, which `_prog` and the version lookup do not want.
          An `api` submodule avoids that; `__all__` in `__init__` does not.

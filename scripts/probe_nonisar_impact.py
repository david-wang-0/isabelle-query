...
def parse(path, thy, with_fix):
    parsing.extract_nonisar_ranges = _real if with_fix else (lambda lines: [])
...
print(f"\nRANGES STARTING ON A DECLARATION (swallowed live source): {len(swallowed)}")
